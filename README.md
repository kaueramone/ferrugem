...
O [refinamento FPS da versão 0.3.0](Docs/REFINAMENTO-FPS.md) acrescenta mira de ferro, agachamento, salto, rampa/escada, mãos e áudio procedural. A preparação passou em 19 verificações isoladas e a regressão de combate em 53, com simulador ativo de 50 ms e perda de 2% por direção. O Motor passou em 60 verificações na build final, com o mesmo perfil de rede; as regressões finais passaram em 53 verificações de combate (com simulador) e 27 de FPS (sem simulador). A inspeção visual parcial confirmou HUD, mãos/arma e transições de morte e renascimento; playtest completo e escuta continuam pendentes. Uma falha transitória de conexão anterior permanece sem causa identificada. Os resultados do parágrafo anterior são da 0.2.0.

## Para começar

- Para testar, feche o Unity e dê dois cliques em **`Testar-Ferrugem.bat`**. O launcher verifica os arquivos do projeto, compila quando necessário e abre dois clientes Windows conectados ao servidor dedicado Linux no WSL. Use WASD, Shift e mouse; Ctrl agacha, Espaço pula e botão direito mira. Esc libera o cursor. No marco 2B, clique dispara, R recarrega e G lança uma carga explosiva. Feche as duas janelas para encerrar o teste. É necessário Ubuntu-24.04 funcionando no WSL 2.
